...
exports.onConnection = function(socket)  {
  ...
  // 接続追加されたら、部屋人数のupdate
  __updateRoomMember(socket, chatroomId);
...
// 部屋人数の更新処理 (connect / disconnect時)
function __updateRoomMember(socket, chatroomId) {
  var io = app.set('io');
  var logger = app.set('logger');
  var total = 0;
  ...
  // Redis で管理している部屋に接続中のユーザー数を取得
  redisClient.hkeys("chatroomClients:"+chatroomId, function(err, clients) {
    clients.forEach(function(sid) {
      total++;
    });
    logger.info('### room_members --- roomId: '+chatroomId);
    logger.info(total);

    // 再度メンバー繰り返し ... 他に良い処理の仕方無いかな ...
    clients.forEach(function(sid) {
      process.nextTick(function() {
        io.sockets.socket(sid).emit('room_members', total);
      });
    });
  });
}
...
exports.onDisconnect = function(socket)  {
  ...
  logger.info('##socket.io: onDisconnect');
